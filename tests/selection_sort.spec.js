...
describe('selection_sort', function () {
  it('should sort an array', (done) => {
   ...
   it('should pass with a very large array', (done) => {
    ...
    expect(selection_sort.selection_sort(array)).to.deep.equal([ 2, 2, 2, 2, 3, 3, 4, 4, 5, 6, 9, 10, 21, 32, 34, 34, 53, 100, 300 ]);
    done();
  })
  it('should sort a decreasing array', (done) => {
    let array = [2000, 803, 566, 230, 112, 99, 98, 97, 81, 55, 43, 32, 23, 22, 21, 19, 32];
    expect(selection_sort.selection_sort(array)).to.deep.equal([ 19, 21, 22, 23, 32, 32, 43, 55, 81, 97, 98, 99, 112, 230, 566, 803, 2000 ]);
    done();
  })
  it('should sort an array with negative values and decimals', (done) => {
    let array = [10, -9, 22, -34, 4, 5.3, 6, 13, -2, 0];
    expect(selection_sort.selection_sort(array)).to.deep.equal([ -34, -9, -2, 0, 4, 5.3, 6, 10, 13, 22 ]);
    done();
  })
